...
#define DEF0(n) DEFINE_ISR_STUB(n, 0)
#define DEF1(n) DEFINE_ISR_STUB(n, 1)
...
extern void (*__isr_stub_table[])(void); /* isr.c で定義 */
void intr_init_all_vectors(void)
{
    ...
    for (int v = 0; v < 256; ++v) {
        idt_set_entry(v, __isr_stub_table[v], cs, 0xE, 0);
    }
